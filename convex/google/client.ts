...
export function createGoogleCalendarClient(
  ...
  return {
    async insertEvent(event) {
      ...
      if (!res.ok) {
        throw new Error(`Google insert failed: ${res.status}`);
      }
    ...
    async updateEvent(eventId, event) {
      ...
      if (!res.ok) {
        throw new Error(`Google update failed: ${res.status}`);
      }
      return (await res.json()) as GoogleCalendarEvent;
    },
    ...
    async listChanges({ syncToken, timeMin }) {
      ...
      if (!res.ok) {
        throw new Error(`Google list failed: ${res.status}`);
      }
    ...
    async watch({ channelId, address, expirationMs }) {
      ...
      if (!res.ok) {
        throw new Error(`Google watch failed: ${res.status}`);
      }
      const data = (await res.json()) as {
        resourceId: string;
        expiration: string;
